...
def _is_onboarding_done(workspace_dir: str) -> bool:
    """Check if AGENT.md has been modified from the original template."""
    agent_path = os.path.join(workspace_dir, DEFAULT_AGENT_FILENAME)
...
- `MEMORY.md` / `memory/YYYY-MM-DD.md` — 用户相关信息与对话结论

⚠️ 只说"记住了"而不调用 edit 写入 = 没有完成。信息只有写入文件才会被持久保存。

## 🎉 全部完成后

当 AGENT.md 的核心字段已填写后，用 bash 执行 `rm BOOTSTRAP.md` 删除此文件。你不再需要引导脚本了——你已经是你了。
"""
